...
// RTMPose normalization — RGB 顺序与模型训练一致
// mean/std 按 RGB 排列：mean=[123.675,116.28,103.53] 对应 [R,G,B]
// 预处理配置: to_rgb=True → 输入给模型的张量通道顺序为 R/G/B
constexpr float kRtmposeMean[3] = {123.675f, 116.28f, 103.53f};  // R, G, B
constexpr float kRtmposeStd[3]  = {58.395f,  57.12f,  57.375f};  // R, G, B
...
bool PoseDetector::loadModel(const QString& onnxPath) {
    ...
    try {
        ...
        // 将名称复制到 std::string 缓冲，再让 const char* 指向缓冲
        // 避免 AllocatedStringPtr::release() 后 allocator 析构导致悬空指针
        m_onnxInputNamesBuf.clear();
        m_onnxOutputNamesBuf.clear();
        m_onnxInputNames.clear();
        m_onnxOutputNames.clear();

        size_t numInputs = m_onnxSession->GetInputCount();
        for (size_t i = 0; i < numInputs; ++i) {
            auto name = m_onnxSession->GetInputNameAllocated(i, allocator);
            m_onnxInputNamesBuf.emplace_back(name.get());   // 深拷贝到 std::string
        }
        for (const auto& s : m_onnxInputNamesBuf)
            m_onnxInputNames.push_back(s.c_str());          // 指向稳定内存

        size_t numOutputs = m_onnxSession->GetOutputCount();
        for (size_t i = 0; i < numOutputs; ++i) {
            auto name = m_onnxSession->GetOutputNameAllocated(i, allocator);
            m_onnxOutputNamesBuf.emplace_back(name.get());  // 深拷贝到 std::string
        }
        for (const auto& s : m_onnxOutputNamesBuf)
            m_onnxOutputNames.push_back(s.c_str());         // 指向稳定内存
        
        // 使用固定输入形状，不依赖模型中可能含 -1（动态轴）的 shape
        // 模型 input_size=(288,384): [batch=1, channels=3, H=384, W=288]
        m_onnxInputShape = {1, 3, static_cast<int64_t>(m_inputH), static_cast<int64_t>(m_inputW)};

        // 获取输出张量形状 [1,17,simcc_dim]
        // simcc_x 形状 [1,K,inputW*split_ratio=576], simcc_y 形状 [1,K,inputH*split_ratio=768]
        auto outputShapeInfoX = m_onnxSession->GetOutputTypeInfo(0).GetTensorTypeAndShapeInfo();
        auto outputShapeX = outputShapeInfoX.GetShape();
        m_numKpts    = static_cast<int>(outputShapeX[1]);
        m_simccXDim  = static_cast<int>(outputShapeX[2]);  // W * split_ratio = 288*2 = 576

        auto outputShapeInfoY = m_onnxSession->GetOutputTypeInfo(1).GetTensorTypeAndShapeInfo();
        auto outputShapeY = outputShapeInfoY.GetShape();
        m_simccYDim  = static_cast<int>(outputShapeY[2]);  // H * split_ratio = 384*2 = 768
...
bool PoseDetector::runInference(const std::vector<float>& inputCHW) {
    ...
    try {
        ...
        // 输出顺序：simcc_x [0], simcc_y [1]，先做边界检查防止越界
        if (outputs.size() < 2) {
            qWarning() << "[PoseDetector] ONNX 输出数量不足:" << outputs.size();
            return false;
        }
        float* simccX = outputs[0].GetTensorMutableData<float>();
        float* simccY = outputs[1].GetTensorMutableData<float>();

        // 防止 cpuOutput 缓冲未初始化时 copy 越界
        if (m_cpuOutputX.empty() || m_cpuOutputY.empty()) {
            qWarning() << "[PoseDetector] cpuOutput 缓冲未初始化";
            return false;
        }
        ...
        std::vector<float>& outCHW) const
{
    ...
    for (int y = 0; y < m_inputH; ++y) {
        for (int x = 0; x < m_inputW; ++x) {
            ...
            for (int c = 0; c < 3; ++c) {
                // to_rgb=True → 通道顺序 R/G/B，Format_RGB888 中 c=0→R,1→G,2→B
                float pix = sampleChannel(rgb, srcX, srcY, c);  // RGB order
